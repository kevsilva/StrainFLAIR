...
Description:
Usage:
    graphs_construction_part1.py (--inSeq <str>) (--inClusters <str>) (--outDir <dir>) [--min_length <float>] [--verbose] [--debug]
...
try:
    ...
    print("graphs_construction_part1.py requires module docopt, to install: pip3 install docopt")
    sys.exit()
...
def main():
    ...
    for cluster in d_clusters:
        print(f"Building graph for {cluster}...")
        with Timer() as _t:
            ...
            if d_clusters[cluster]['len_rep'] < min_length:
                ...
                if len(d_clusters[cluster]['genes_list']) == 1: # if only one sequence in the cluster, just build a linear graph with vg construct
                    subprocess.check_call(f"vg construct -r {cwd}/gc_tmp/cluster_temp.fasta -m 256 | vg ids -c -i {N} - > {output_dir}/{cluster}.vg",stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
                    subprocess.check_call(f"rm -r {cwd}/gc_tmp/*.fai",shell=True)
                else:
                    subprocess.check_call(f"minimap2 -cx asm20 -X -t 8 {cwd}/gc_tmp/cluster_temp.fasta {cwd}/gc_tmp/cluster_temp.fasta | gzip > {cwd}/gc_tmp/cluster_temp.paf.gz",stdout=subprocess.PIPE,stderr=subprocess.PIPE,shell=True)
                    ...
                    subprocess.check_call(f"vg view -Fv {cwd}/gc_tmp/cluster_temp.gfa | vg mod -n -X 256 - | vg ids -i {N} -c - | vg sort - > {output_dir}/{cluster}.vg",shell=True)
                    # -n can mess up the graph, check for its integrity otherwise redo the graph without -n
                    p1 = subprocess.Popen(["vg","validate",f"{output_dir}/{cluster}.vg"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    out,err = p1.communicate()
                    if err.decode() != "":
                        subprocess.check_call(f"vg view -Fv {cwd}/gc_tmp/cluster_temp.gfa | vg mod -X 256 - | vg ids -i {N} -c - | vg sort - > {output_dir}/{cluster}.vg",shell=True)
